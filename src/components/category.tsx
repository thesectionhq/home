...
export default function Category({data: pathData}: any) {
  ...
  useEffect(() => {
    async function handleFetchArticles() {
      try {
        const response = await axios.get(`/api/content/articles?category=${pathData?.category}`);
  ...
  return (
    <div className="container mx-auto px-4 py-8 mt-15">
      <div className="w-full flex items-center justify-center md:justify-between  md:py-10">
        <h1 className="text-[40px] text-center font-secondary font-bold uppercase">{pathData.category}</h1>
        <Filter />
      </div>
